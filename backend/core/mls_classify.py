"""
MLS Classifier — Market Lens (Cloud-first)

Responsabilidade:
- Classificar linhas MLS por ASSINATURA DE COLUNAS (determinístico)
- Mapear status (listing / pending / closed)
- Normalizar preços (list_price / close_price)
- NÃO contém lógica de negócio além do contrato
"""
...
# =========================================================
# Asset class inference (DETERMINISTIC)
# =========================================================

def infer_asset_class(columns: List[str], contract: dict) -> str:
    cols = set(columns)

    rental_sig = set(contract["signatures"]["rental_if_has_any"])
    land_sig = set(contract["signatures"]["land_if_has_any"])

    if rental_sig.intersection(cols):
        return "rental"

    if land_sig.intersection(cols):
        return "land"
...
def map_status(asset_class: str, status_raw: str, contract: dict) -> Tuple[str, Optional[str]]:
...
# =========================================================
# Price normalization (via contract)
# =========================================================
...
def classify_xlsx(
    ...
    """
    Reads an MLS XLSX file and returns a classified DataFrame
    ready for insertion into stg_mls_classified.
    """
    ...
    for _, r in df.iterrows():
        ...
        list_price, close_price = normalize_price(
            asset_class,
            status_raw,
            r.get("Current Price"),
        )
        ...
        row = {
            ...
            "property_subtype": (
                "Vacant Land"
                if asset_class == "land"
                else clean_string(r.get("Property Style"))
            ),
